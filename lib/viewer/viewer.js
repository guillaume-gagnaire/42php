function 							Viewer(viewer_id, slide) {
	this.confEl = $("#" + viewer_id);
	if (!this.confEl)
		return;
	...
	(function(o){
		o.confEl.find('img').each(function(){
			o.images.push({
				url: this.src,
				height: this.height,
				width: this.width
		...
		var c = o.confEl.find('[data-column]:first');
		if (c.length)
			o.col = c[0];
	})(this);
...
$(function(){
	$(document).on('click', '[data-bind-viewer]', function(){
		var offset = $(this).attr('data-viewer-offset');
		new Viewer($(this).attr('data-bind-viewer'), typeof offset == "undefined" ? 0 : parseInt(offset));
	});
});